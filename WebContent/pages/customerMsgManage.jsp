								...
								<div class="orderopt-right">
										...
										<tr>
											<td colspan="4">
												...
												<button type="button" class="btn btn-info btn-xs" title="联系电话"  
														...
														data-content="${orderopt.orderoptPaycustomer.cusPhone}">
													联系买家
												</button>
									...
									<div class="orderoptfoot">
										...
										<div class="orderoptfoot-right">
											<c:if test="${orderopt.orderoptStatus == '待发货'}">
  												<button type="button" onclick="deliverGoods('${orderopt.orderoptId}',this)" class="btn btn-danger btn-xs">确认发货</button>
  												<hr style="margin: 4px">
  												<button type="button" onclick="overOrder('${orderopt.orderoptId}',this)" class="btn btn-default btn-xs">关闭订单</button>
											</c:if>
										</div>
									</div>
								</div>
...
<script type="text/javascript">
	 ...
	 function bindScrollEvent(){ 
		...
		$(window).scroll(function(){
			...
			if(docHeight==winHeight+winScorllHeight){
				...
				$.ajax({ 
					   type : "POST", //提交方式 
					   url : "${pageContext.request.contextPath}/findMsgManage",//路径
    	...
    	function returnTemp(data){
			...
			for(var i = 0;i < data.list.length;i++){
				...
				temp = temp + "data-content='"+data.list[i].orderoptPaycustomer.cusPhone+"'>";
				temp = temp + "联系买家</button>";
				temp = temp + "</td>";
				temp = temp + "</tr>";
				temp = temp + "<tr>";
				...
				if(data.list[i].orderoptStatus=="待发货"){
					temp = temp + '<button type="button" onclick="deliverGoods('+"'"+data.list[i].orderoptId+"'"+',this)" class="btn btn-danger btn-xs">确认发货</button>';
					temp = temp + "<hr style='margin: 2px;'/>";
					temp = temp + '<button type="button" onclick="overOrder('+"'"+data.list[i].orderoptId+"'"+',this)" class="btn btn-default btn-xs">关闭订单</button>';
				}				
				temp = temp + "</div>";
				temp = temp + "</div>";
				temp = temp + "</div>";
    ...
    //确认发货
    function deliverGoods(orderoptId,_this){
    	var _this = $(_this);
    	$.ajax({
			type : "POST", //提交方式 
			url : "${pageContext.request.contextPath}/deliverGoods",//路径
			dateType : "json",//返回的数据类型
			data : {
    ...
    //关闭订单
    function overOrder(orderoptId,_this){
    	var _this = $(_this);
    	$.ajax({
			type : "POST", //提交方式 
			url : "${pageContext.request.contextPath}/overOrder",//路径
			dateType : "json",//返回的数据类型
			data : {
				"orderoptId":orderoptId
			},
